...
DESCRIPTION:
Order book snapshot — protocol-common across every Bybit V5 category.
Returned by MarketData.GetOrderBook and pushed by the WebSocket
"orderbook" topic with type=="snapshot".

Bybit synchronisation model:
  - Each push (snapshot or delta) carries an "u" sequence number;
    deltas are applied in order and a missing u indicates a gap that
  ...
  - Bybit does NOT publish CRC32 checksums (unlike OKX) — gap
    detection is purely sequence-based.
